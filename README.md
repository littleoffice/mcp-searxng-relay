...
| `mcp_search_degraded_total` | — | `searxng_web_search` calls that returned HTTP 200 but named unresponsive engines. **Not** an error, so `mcp_search_errors_total` does not see them — yet the agent answered from a thinner result set. A rising rate against `mcp_searches_total` is the signal that upstream answer quality is eroding; the named engines are in the WARN log |
