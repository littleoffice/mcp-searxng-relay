...
func runHealthCheck() {
	...
	// probe must speak TLS too. Certificate verification is skipped on purpose:
	// this is a loopback self-check, not a trust boundary, and the served
	// certificate is issued for the public hostname(s), not 127.0.0.1 — so a
	// verifying probe would always fail. The env vars are the same ones
	// newTLSSettings reads, so a single configuration covers both.
	scheme := "http"
	client := &http.Client{Timeout: 5 * time.Second}
	if healthProbeUsesTLS() {
		scheme = "https"
		client.Transport = &http.Transport{
			TLSClientConfig: &tls.Config{InsecureSkipVerify: true}, //nolint:gosec // loopback self-probe; see comment above
		}
	}
