...
Every tool response is an awareness preamble, a blank line, then one fence
element:
...
Three properties of this layout are load-bearing for a verifier:
...
| `rating` | yes | yes | `trusted` \| `partially-trusted` \| `untrusted`. Always `untrusted` today. |
| `timestamp` | yes | yes | RFC 3339, UTC. |
| `type` | yes | yes | `instructions` \| `content` \| `data`. |
| `kid` | yes | yes | Key fingerprint; matches `/fence/public-key`'s `fingerprint`. |
| `version` | yes | yes | Fence format version, currently `1.0`. |
| `source` | when known | yes | The fetched URL. Absent for responses with no single source. |
...
```json
{"version":"1.0","algorithm":"Ed25519","publicKey":"<base64>","fingerprint":"6fad8756da085aac"}
```
...
- **The awareness preamble is unsigned.** It sits outside the fence, and it is
  the text instructing the model to treat the fenced content as data. Nothing
  binds it to the fence it describes. Untrusted content cannot currently reach
  it — on the escaped path the content is escaped and enclosed — so this is not
  presently exploitable here. But the mechanism it protects is defeated by
  editing it, without touching a signature, and §4.2 assumes every segment is
  fenced. A verifier should be able to report unsigned regions; the real fix is
  for the generator to wrap the preamble in its own
  `rating="trusted" type="instructions"` fence, which is what the paper
  prescribes for system instructions anyway.
...
| Preamble prose `<sec:fence` occurrences | ignored, not counted as rejections |

To generate vectors against the real implementation, call `wrapFence` and
`wrapFenceCDATA` directly — `fence.go` depends only on the standard library and
